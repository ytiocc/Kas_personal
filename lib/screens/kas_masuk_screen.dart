...
void onItemTap(Kas kas) async{
  await showDialog(
    ...
    content: const Text('Yakin pingin hapus dek?'),
    actions: [
      ElevatedButton(
        onPressed: (){
      ...
      child: const Text('Ya')),
      ElevatedButton(onPressed: () => Navigator.of(context).pop, child: const Text('Tidak')),
    ], 
      ),
    );
}
...
class _BaruDialogState extends State<BaruDialog> {
  ...
  final keteranganController =TextEditingController(text: '');
   ...
   var kas =Kas.masuk(keterangan: keteranganController.text,
    nominal: nominal, 
